...
class GraphRagConfig(BaseModel):
    ...
    config_dir: str = Field(
        description="The config directory for the configuration.",
        default=graphrag_config_defaults.config_dir,
    )

    def _validate_config_dir(self) -> None:
        """Validate the root directory."""
        if self.config_dir.strip() == "":
            self.config_dir = str(Path.cwd())

        config_dir = Path(self.config_dir).resolve()
        if not config_dir.is_dir():
            msg = f"Invalid root directory: {self.config_dir} is not a directory."
